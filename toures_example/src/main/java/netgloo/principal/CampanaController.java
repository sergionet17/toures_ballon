...
public class CampanaController {
    ...
    public Campaña getUser(@PathVariable("email") int email) throws IndexOutOfBoundsException {
    ...
    // Metodo para obtner la campaña segun el email
    //localhost:8080/user/12
    @RequestMapping(value = "/campanas/{id}", method = RequestMethod.GET)
    public Campaña getCampana(@PathVariable("id") int id) throws IndexOutOfBoundsException {
        Campaña campaña = new Campaña();
        try {
            List<Campaña> listCampaña = new ArrayList<Campaña>();
            listCampaña = campañaDao.getById(id);
            campaña = listCampaña.get(0);
            System.err.println(campaña.getName());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return campaña;
